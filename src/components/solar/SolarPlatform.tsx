...
function Monitor() {
  const voltage: EChartsOption = { legend: { top: 10, right: 10, textStyle: { color: "#8aa0b7" }, data: ["A相", "B相", "C相"] }, grid: baseGrid, xAxis: { type: "category", data: times, ...axis }, yAxis: { type: "value", min: 215, max: 245, splitLine: { lineStyle: { color: "rgba(184,203,224,.08)" } }, ...axis }, series: [cyan, gold, green].map((color, i) => ({ name: ["A相", "B相", "C相"][i], type: "line", smooth: true, symbol: "none", lineStyle: { color, width: 2 }, data: power.map((v, n) => 226 + ((v + n * (i + 2)) % 12)) })) };
